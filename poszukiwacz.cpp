...
int GetReturnValue(int duplicateCounter, const long size){
    return ((duplicateCounter/(double)size) * 100 + 5) / 10;
}
...
//     this sets errno so we have to reset it to 0
    ...
    short* Numbers = (short*)calloc(ParsedArgument*sizeof(short), 1);
    ...
    for (size_t i = 0; i < ParsedArgument; i++) {
        ...
        if(CheckIfExistsInArray(Numbers,i, num)){
            ...
            fprintf(stderr, "DIGIT: %hd \tPID: %d\n", num, GetCurrentPid());
            Check(fwrite(&record,sizeof(record), 1, stdout), "Couldn't write.\n");
        }
    }
